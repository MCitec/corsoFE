...
import { formatCurrency, formatDate, transactions } from "./utils.js";
...
function loadAccount() {
  return fetchAccount(CURRENT_ACCOUNT_ID).catch((error) => {
    if (error.statusCode !== 0) throw error;
    return localAccount;
  });
}

function loadTransactions() {
  return fetchTransactions(CURRENT_ACCOUNT_ID).catch((error) => {
    if (error.statusCode !== 0) throw error;
    return transactions;
  });
}
...
async function initDashboard() {
  ...
  const balanceRequest = loadAccount()
    .then(renderBalanceWidget)
    .catch((error) => {
      const balanceEl = document.getElementById("balanceAmount");
      if (balanceEl) balanceEl.textContent = "Saldo non disponibile";
